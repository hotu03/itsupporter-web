/**
 * Firestore Migration Verification Script (CommonJS version)
 *
 * Run with: node verify-firestore-migration.cjs
 */
...
if (fs.existsSync(legacyDir)) {
  ...
  localStorageModules.forEach(module => {
    ...
    if (legacyFiles.some(f => f.includes(expectedFile.replace('.ts', '')))) {
      console.log(`  ✅ ${expectedFile}`);
    } else {
      console.log(`  ⚠️  Missing or renamed: ${expectedFile}`);
      warnings++;
    }
  });
} else {
...
// 2. Check that no source files import old localStorage modules directly (except legacy and test files)
...
sourceFiles.forEach(file => {
  ...
  localStorageModules.forEach(module => {
    ...
    if (importPattern.test(content) &&
        !file.includes('legacy-localstorage') &&
        !file.includes('test') &&
...
if (warnings === 0) {
  console.log('✅ No direct localStorage imports found in active source code');
} else {
  console.log(`⚠️  Found ${warnings} potential old imports (some may be acceptable in legacy or test files)`);
}

// 3. Verify all firestore modules exist
...
// 4. Check for console.log in production firestore files
console.log('\n🔍 Checking for console statements in Firestore files...');
...
if (errors === 0) {
  ...
  console.log('\n✅ All data layers are now using Firestore');
  console.log('✅ Legacy code safely archived in: data/legacy-localstorage/');
  console.log('\n📊 Summary:');
  console.log(`   - ${firestoreModules.length} Firestore modules verified`);
  console.log(`   - ${localStorageModules.length} legacy implementations archived`);
  console.log('   - All active code paths use Firestore');
} else {
  console.log(`❌ MIGRATION VERIFICATION FAILED with ${errors} errors`);
}

console.log('\nTo run this test again: node verify-firestore-migration.cjs');
